$(document).ready(function(){
	...
	$('.button').bind('click', function(event){
		...
		if(len<1&&len!==""){
			...
			$('.conf-password-error').text('Confirme la contrasenia');
		}
		else{
			...
			if($('.password').val() !== $('.conf-password').val()){
				$('.conf-password').next().show();
				$('.conf-password-error').next().show();
				$('.conf-password-error').text('La contrasenia no es igual');
			}
			else{
				$('.conf-password').next().hide();
				$('.conf-password-error').hide();
		...
		if(count===0){
			...
			if(count=="0"){
				...
				$('#buttons').append('<a id="info" href="#openModal">Ver detalle del pedido</a>');
	...
	$(document).on('click', '#info', function(){
		...
		$('#openModal > div').append(
			...
			'<li> La ciudad donde ve es: '+ localStorage.getItem('city')+ '</li></ul>'
